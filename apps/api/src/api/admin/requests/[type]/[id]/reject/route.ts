...
export async function POST(
  ...
  if (submitterId) {
    ...
    const notified = await notifyMessengerUser({
      ...
      senderName: "Kayı.com",
      preview: interpolateNotification(messages.request.rejectedPreview, { label }),
    ...
    if (!notified) {
      logger.warn(`[requests/reject] Messenger notify was not accepted for ${alias} ${entityId}`)
    }
  }
