...
export default async function returnNotificationNewReturnSubscriber({
  ...
  try {
    ...
    // the customer who requested the return has no reason to see it
    // reflected back at them, so they're never made a conversation
    // participant here (regardless of whether they have a real account or
