...
export const handle = {
  breadcrumb: () => i18n.t("requests.tags.navLabel"),
};

import { useProductTagRequests } from "../../../hooks/api/requests";
...
const VendorTagRequestsPage = () => {
  ...
  const { requests, count, isError, error, isLoading } = useProductTagRequests(
    searchParams,
    ...
    <SingleColumnPage>
      <Container className="divide-y p-0">
        <div className="flex items-center justify-between px-6 py-4">
          <Heading>{t("requests.tags.heading")}</Heading>
        ...
        <_DataTable
          ...
          queryObject={raw}
          noRecords={{ message: t("requests.tags.noRecords") }}
        />
      </Container>
    </SingleColumnPage>
  );
