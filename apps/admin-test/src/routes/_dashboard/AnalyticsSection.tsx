...
type MetricKey = "orders_count" | "commission_earnings"
...
export const AnalyticsSection = ({
  ...
  const totals = trend.reduce(
    (acc, point) => ({
      ...
      commission_earnings: acc.commission_earnings + point.commission_earnings,
    }),
    { orders_count: 0, commission_earnings: 0 }
  )
      ...
      <div className="grid grid-cols-1 gap-4 px-6 py-4 lg:grid-cols-4">
                ...
                <Tooltip
                  formatter={(value: number, key: string) => [
                    ...
                    key === "orders_count"
                      ...
                      : t("dashboard.analytics.commissionEarnings"),
                  ]}
                />
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:block">
          ...
          <button
            type="button"
            className="hover:bg-ui-bg-base-hover transition-fg my-1 flex w-full flex-col items-start rounded-lg border p-4 text-left"
            onClick={() => toggleMetric("commission_earnings")}
          >
            <Heading level="h3">{t("dashboard.analytics.commissionEarnings")}</Heading>
            <div className="mt-2 flex items-center gap-2">
              <div
                className="h-8 w-1 rounded"
                style={{
                  backgroundColor: activeMetrics.includes("commission_earnings")
                    ? lineColor("commission_earnings")
                    : "var(--border-strong, #a1a1aa)",
                }}
              />
              <Text className="text-ui-fg-subtle">
                {totals.commission_earnings.toFixed(2)}
              </Text>
            </div>
          </button>
        </div>
